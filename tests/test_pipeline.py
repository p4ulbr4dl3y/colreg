...
@pytest.mark.parametrize('image_name, expected_type', [('cbd.png', 'CBD'), ('fishing.png', 'FISH'), ('nuc.png', 'NUC'), ('power-driven.webp', 'MECH'), ('ram.png', 'RAM'), ('sailing.jpg', 'SAIL')])
def test_day_mode_classifications(pipeline, image_name, expected_type):
    ...
    found_expected = any((boat.final_vessel_type == expected_type for boat in result.boats))
    assert found_expected, f'Ожидаемый тип {expected_type} не найден на {image_name}. Найденные типы: {[b.final_vessel_type for b in result.boats]}'

@pytest.mark.parametrize('category, expected_type', [('cbd', 'CBD'), ('fishing', 'FISH'), ('nuc', 'NUC'), ('power-driven', 'MECH'), ('ram', 'RAM'), ('sailboat', 'SAIL')])
def test_night_mode_classifications(pipeline, category, expected_type):
    ...
    found_expected = any((boat.final_vessel_type == expected_type for boat in result.boats))
    assert found_expected, f'Ожидаемый тип {expected_type} не найден в ночной категории {category}. Найденные типы: {[b.final_vessel_type for b in result.boats]}'
