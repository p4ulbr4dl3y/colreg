...
class VisionNode:
    ...
    def __init__(self):
        ...
        self.client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, 'colreg_vision_node')
    ...
    def _handle_analyze_command(self, payload: Dict[str, Any]) -> None:
        ...
        if not source:
            self._publish_error(request_id, "Отсутствует 'source' в полезной нагрузке команды.")
            return
        ...
        try:
            ...
            if image is None:
                self._publish_error(request_id, f'Не удалось загрузить изображение из источника: {source}')
                return
            result = self.pipeline.process(image, is_night=is_night)
            boats_data = []
            for boat in result.boats:
                boats_data.append({'boat_id': boat.boat_id, 'bbox': boat.bbox, 'vessel_type': boat.final_vessel_type, 'confidence': round(boat.final_vessel_type_confidence, 2)})
            response = {'request_id': request_id, 'status': 'success', 'is_night': result.is_night, 'boat_count': result.boat_count, 'boats': boats_data, 'processing_time_ms': round((time.time() - start_time) * 1000, 2)}
            self.client.publish(MQTT_TOPIC_RESULT, json.dumps(response))
            logger.info(f'[{request_id}] Анализ завершен. Найдено судов: {result.boat_count}. Опубликовано в {MQTT_TOPIC_RESULT}')
        except Exception as e:
    ...
    def _publish_error(self, request_id: str, error_message: str) -> None:
        response = {'request_id': request_id, 'status': 'error', 'message': error_message}
        self.client.publish(MQTT_TOPIC_RESULT, json.dumps(response))
