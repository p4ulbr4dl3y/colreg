...
def _group_by_mast(
    ...
    Использует эвристики по X, Y и консистентности размеров огней.

    Args:
        detections: Список обнаруженных огней.
        x_tolerance: Максимальное горизонтальное расстояние для считания одной мачтой.
        max_y_gap_factor: Максимальный вертикальный разрыв (в средних высотах огней).
    ...
    def get_area(d: LightDetection) -> float:
    ...
    for i in range(1, len(sorted_detections)):
        ...
        y_gap = curr.bbox[1] - prev.bbox[3]  # Расстояние между огнями
