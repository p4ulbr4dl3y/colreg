...
DAY_SHAPES_RULES = {VesselType.NUC: {'sequence': [0, 0], 'color': (0, 0, 255), 'description': 'Не может управляться - 2 шара'}, VesselType.RAM: {'sequence': [0, 3, 0], 'color': (170, 255, 170), 'description': 'Ограничено в возможности маневрировать - шар-ромб-шар'}, VesselType.CBD: {'sequence': [4], 'color': (0, 165, 255), 'description': 'Стеснено своей осадкой - цилиндр'}, VesselType.FISHING: {'sequence': [2, 1], 'color': (0, 255, 255), 'description': 'Занято ловом рыбы - конусы вершинами вместе'}}

def _group_by_mast(detections: List[DayShapeDetection], x_tolerance: int=40, max_y_gap_factor: float=4.0, max_area_ratio: float=4.0) -> List[List[DayShapeDetection]]:
    if not detections:
        return []
...
def _classify_group(group: List[DayShapeDetection], rules: dict=DAY_SHAPES_RULES) -> VesselTypeResult:
    ...
    return VesselTypeResult(vessel_type=vessel_type, bbox=[x1_min, y1_min, x2_max, y2_max], color=color, shapes=group, sequence=sequence)

def classify_day_shapes(image: Union[str, Path, np.ndarray], config: Optional[Config]=None, confidence_threshold: Optional[float]=None, model_path: Optional[Union[str, Path]]=None, x_tolerance: Optional[int]=None, return_detections: bool=False, model: Optional[YOLO]=None) -> Union[List[VesselTypeResult], Tuple[List[VesselTypeResult], List[DayShapeDetection]]]:
    if config is None:
        config = Config()
    ...
    results = model(image, conf=confidence_threshold or config.day_shapes.confidence_threshold, device=config.device, verbose=False)
    result = results[0]
    detections = []
    if result.boxes is not None:
        ...
        for i in range(len(boxes)):
            ...
            detections.append(DayShapeDetection(class_id=class_id, class_name=config.day_shapes_classes.get(class_id, f'class_{class_id}'), bbox=[x1, y1, x2, y2], center_x=(x1 + x2) / 2, center_y=(y1 + y2) / 2, confidence=conf))
