...
def expand_bbox(bbox: List[int], image_shape: Tuple[int, ...], bbox_scale: Tuple[float, float, float, float]) -> List[int]:
    (x1, y1, x2, y2) = bbox
    cx = (x1 + x2) / 2
    cy = (y1 + y2) / 2
    w = x2 - x1
...
class PipelineResult:
    ...
    def mechanical_count(self) -> int:
        from colreg_vision.core.types import VesselType
        return sum((1 for b in self.boats if b.final_vessel_type == VesselType.MECHANICAL))

class VideoAnalyticsPipeline:

    def __init__(self, config: Optional[Config]=None, classifier_device: Optional[str]=None):
    ...
    def classifier(self) -> BinaryClassifier:
        if self._classifier is None:
            self._classifier = BinaryClassifier(config=self.config, device=self._classifier_device)
    ...
    def infrared_detector(self) -> YOLO:
        if self._infrared_detector is None:
            self._infrared_detector = YOLO(str(self.config.get_model_path('infrared_detector')))
    ...
    def process(self, image: Union[str, Path, np.ndarray], is_night: bool=False, boat_confidence: Optional[float]=None, classifier_confidence: Optional[float]=None, skip_classification: bool=False, bbox_scale: Tuple[float, float, float, float]=(1.0, 5.0, 1.0, 1.0)) -> PipelineResult:
        ...
        boat_detections = detect_and_crop_boats(image=image, config=self.config, confidence_threshold=boat_confidence, model=self.boat_detector, use_tracker=self.config.use_tracker)
        boats = []
        ...
        for boat_det in boat_detections:
            ...
            boat_result = BoatAnalysisResult(boat_id=boat_det.crop_id, crop=crop, bbox=exp_bbox, detection_confidence=boat_det.confidence)
        ...
        if not skip_classification and crops_for_classification:
            ...
            for (boat, class_result) in zip(boats, class_results):
                if class_result.is_sailboat:
                    ...
                    boat.vessel_type_confidence = class_result.not_sailboat_probability * 100
        result = PipelineResult(image=image, is_night=is_night, boats=boats)
        if is_night:
            result.infrared_detections = detect_infrared_objects(image=image, config=self.config, model=self.infrared_detector, use_tracker=self.config.use_tracker)
            for boat in boats:
                if boat.crop.size == 0:
                    continue
                lights_statuses = classify_lights(image=boat.crop, config=self.config, model=self.lights_model)
                valid_statuses = []
            ...
            for boat in boats:
                day_statuses = classify_day_shapes(image=boat.crop, config=self.config, model=self.day_shapes_model)
                valid_statuses = []
    ...
    def process_night(self, ir_image: Union[str, Path, np.ndarray], visible_image: Union[str, Path, np.ndarray], boat_confidence: Optional[float]=None, classifier_confidence: Optional[float]=None, skip_classification: bool=False, bbox_offset: Tuple[int, int]=(0, 0), bbox_scale: Tuple[float, float, float, float]=(1.0, 5.0, 1.0, 1.0)) -> PipelineResult:
        ...
        ir_detections_raw = detect_infrared_objects(image=ir_image, config=self.config, confidence_threshold=boat_confidence, model=self.infrared_detector, use_tracker=self.config.use_tracker)
        boat_detections = []
        for ir_det in ir_detections_raw:
            (x1, y1, x2, y2) = ir_det.bbox
            crop = ir_image[y1:y2, x1:x2].copy()
            boat_detections.append(BoatDetection(crop=crop, bbox=ir_det.bbox, confidence=ir_det.confidence, crop_id=ir_det.track_id))
        boats = []
        crops_for_classification = []
        for boat_det in boat_detections:
            exp_bbox = expand_bbox(boat_det.bbox, ir_image.shape, bbox_scale)
            (exp_x1, exp_y1, exp_x2, exp_y2) = exp_bbox
            adj_bbox = [max(0, exp_x1 + bbox_offset[0]), max(0, exp_y1 + bbox_offset[1]), max(0, exp_x2 + bbox_offset[0]), max(0, exp_y2 + bbox_offset[1])]
            ir_crop = ir_image[exp_y1:exp_y2, exp_x1:exp_x2]
            adj_crop = visible_image[adj_bbox[1]:adj_bbox[3], adj_bbox[0]:adj_bbox[2]]
            boat_result = BoatAnalysisResult(boat_id=boat_det.crop_id, crop=adj_crop, bbox=exp_bbox, detection_confidence=boat_det.confidence)
            boats.append(boat_result)
...
def draw_results(image: np.ndarray, result: PipelineResult, thickness: int=2, font_scale: float=0.6) -> np.ndarray:
    output = image.copy()
    for boat in result.boats:
        ...
        ((label_w, label_h), baseline) = cv2.getTextSize(full_label, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
        text_x = x1 + thickness + 2
        text_y = y1 + label_h + thickness + 2
        if text_y > y2:
            text_y = y2 - baseline
        cv2.rectangle(output, (x1, y1), (x1 + label_w + thickness * 2 + 4, y1 + label_h + baseline + thickness * 2 + 4), color, -1)
        cv2.putText(output, full_label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255) if sum(color) < 400 else (0, 0, 0), thickness)
    return output